...
function decorateIngredients(ingredients, library) {
  ...
  // 06-02: flatIndex is a cross-group, render-order-stable per-line index used
  // by the inline-Fix Cancel route to round-trip from a single recipe-page
  // <li> back through decorateIngredients to the same item. Computed here
  // because Nunjucks {% set %} is loop-local and cannot thread a counter across
  // the nested {% for group %}{% for ing %} loops in recipe-ingredient-groups.njk.
  let flatIndex = 0;
